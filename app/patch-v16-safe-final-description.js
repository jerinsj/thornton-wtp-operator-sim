function patchSimulatorSourceV16SafeFinalDescription(source){
  ...
  if(!source.includes(methodology))throw new Error('V16.12.3.1 could not locate the existing end-of-shift methodology text.');
  ...
  if(!source.includes(oldLine))throw new Error('V16.12.3.1 could not locate the existing final Chemical Control description.');
  ...
  const newBlock=`      const completedTasks=ROUTINE_TASKS.filter(t=>t.completed).length;
      ...
      let assessmentTitle='NEEDS IMPROVEMENT';
      let assessmentText='Critical duties or process control fell short. This shift would require significant improvement before being considered satisfactory.';
